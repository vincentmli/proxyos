<?php
	...
	if ($vev_action == "CANCEL") {
		...
		header("Location: iptables_filter_output_edit.php?selected_host=$selected_host");
		/* Make sure that code below does not get executed when we redirect. */
		exit;
	}

	if (($selected_host == "")) {
		header("Location: iptables_filter_output.php");
		exit;
	}
	...
	if ( $vev_action == "ACCEPT" ) {
		...
		foreach($ruleArray_temp as $key => $value) {
			if (empty($value)) unset($ruleArray_temp[$key]);
	     	...
	     	$ipt->replaceRule("filter", "OUTPUT", $selected_host, $ruleArray_temp);
	     	$ipt->applyNow(false, NULL, $rules_file);

		header("Location: iptables_filter_output.php?selected_host=$selected_host");
...
<TITLE>Piranha (IPTABLES OUTPUT - Editing IPTABLES OUTPUT rule)</TITLE>
...
<TABLE WIDTH="100%" BORDER="0" CELLSPACING="0" CELLPADDING="5">
        <TR>
            <TD>&nbsp;<BR><FONT SIZE="+2" COLOR="#CC0000">EDIT FILTER OUTPUT</FONT><BR>&nbsp;</TD>
        </TR>
</TABLE>
...
<TABLE WIDTH="100%" BORDER="0" CELLSPACING="0" CELLPADDING="5">
        <TR BGCOLOR="#EEEEEE">
                <TD WIDTH="60%">MENU:
                <A HREF="iptables_filter_output.php" NAME="FILTER OUTPUT">FILTER OUTPUT</A>
                &nbsp;|&nbsp;
		
		<A HREF="iptables_filter_output_edit.php<?php if (!empty($selected_host)) { echo "?selected_host=$selected_host"; } ?> " CLASS="tabon" NAME="FILTER OUTPUT EDIT">EDIT FILTER OUTPUT</A>
		&nbsp;|&nbsp;
...
<FORM METHOD="GET" id="filter_output_form" ENCTYPE="application/x-www-form-urlencoded" ACTION="iptables_filter_output_edit.php">
<TABLE>
	<?php
		$ruleArray = $ipt->getRule("filter", "OUTPUT", $selected_host);
/*

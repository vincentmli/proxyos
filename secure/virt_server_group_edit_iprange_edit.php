<?php
	...
	if ($edit_action == "CANCEL") {
		header("Location: virt_server_group_edit_iprange.php?selected_host=$selected_host&selected=$selected");		
		exit;
	}
	...
	if ($edit_action == "ACCEPT") {
		...
		$ip	=	$_GET['ip'];
		$port	=	$_GET['port'];
		$virt_server_group[$selected_host]['iprange'][$selected-1] = "$ip" . " " . "$port";	
...
<FORM id="virt_server_group_iprange_form" METHOD="GET" ENCTYPE="application/x-www-form-urlencoded" ACTION="virt_server_group_edit_iprange_edit.php">


	<TABLE>

	<?php	
		$iprange =  $virt_server_group[$selected_host]['iprange'][$selected-1];
		...
		echo "<TR>";
			echo "<TD>PORT: </TD>";
			echo "<TD><INPUT TYPE=TEXT NAME=port VALUE=\""; echo $port . "\""  . ">"; echo "</TD>";
		echo "</TR>";
